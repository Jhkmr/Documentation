...
// overflow text
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    const boxes = Array.from(document.querySelectorAll('.flow-box'));

    const tempSpan = document.createElement('span');
    tempSpan.style.visibility = 'hidden';
    tempSpan.style.position = 'absolute';
    tempSpan.style.whiteSpace = 'normal';
    tempSpan.style.width = boxes[0].clientWidth + 'px';
    tempSpan.style.font = window.getComputedStyle(boxes[0]).font;
    document.body.appendChild(tempSpan);

    let textToPlace = sourceText;

    boxes.forEach(box => {
      let words = textToPlace.split(' ');
      let fitText = '';

      for (let i = 0; i < words.length; i++) {
        const testText = fitText + words[i] + ' ';
        tempSpan.innerText = testText;

        if (tempSpan.offsetHeight > box.clientHeight) {
          break;
        }
      ...
      box.innerText = fitText.trim();
      textToPlace = textToPlace.substring(fitText.length).trim();
    });
