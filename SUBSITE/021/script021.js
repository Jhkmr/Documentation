...
//remove wrappers for print preview
...
function beforePrint() {
  if (wrapper) {
    ...
    // Move all children of .wrapper before removing it
...
function afterPrint() {
  if (wrapper && originalParent) {
    // Restore wrapper and move children back inside
    const newWrapper = document.createElement('div');
    newWrapper.className = 'wrapper';

    // Move all formerly unwrapped nodes back into the wrapper
    let node = originalParent.firstChild;
    while (node && node !== originalNextSibling) {
